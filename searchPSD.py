...
def update_listbox_from_queue():
    while not update_listbox_queue.empty():
        merged_dict = update_listbox_queue.get()
    ...
    tkRoot.after(1000, lambda: update_listbox_from_queue())
...
def process_psd_file(psd_file_path, compare_with, args):
    ...
    for layer in psd:
        ...
        if merged_image:
            ...
            #merged_image.save(merged_image_name)
            result = compare_images_in_memory(compare_with, merged_image, safe_psd_file_path, merged_image_name, processed_files, scale_value, lock, update_listbox_queue)
    ...
    compare_images_in_memory(compare_with, empty_image, safe_psd_file_path, merged_image_name_Merged, processed_files, scale_value, lock, update_listbox_queue)     
...
def process_psd_preview(psd_file_path, compare_with, args):
    ...
    try:
        ...
        return compare_images_in_memory(compare_with, preview_image, safe_psd_file_path, "Preview_Image", processed_files, scale_value, lock, update_listbox_queue)
...
def update_dict(processed_files, scale_value, lock, update_listbox_queue, path, imageName, image, similarity):
    temp_dict = {};
    temp_dict[path] = {}
    temp_dict[path][imageName] = {"similarity": similarity, "image": image}
    log_message(temp_dict)
    #update_listbox_queue.put(temp_dict)
...
def find_template_in_image(image_path, mergedImage, PsdPath, NamePath, args):
    ...
    if similarity >= float(scale_value.value):
        ...
        return update_dict(processed_files, scale_value, lock, update_listbox_queue, PsdPath, NamePath, mergedImage, similarity);
    else:
...
def compare_images_in_memory(image1_path, image2, PsdPath, NamePath, processed_files, scale_value, lock, update_listbox_queue):
    ...
    if similarity >= float(scale_value.value):
        log_message(f"similarity: {similarity} scale_value: {scale_value} == OK ==")
        return update_dict(processed_files, scale_value, lock, update_listbox_queue, PsdPath, NamePath, none_convert, similarity);
    else:
        log_message(f"similarity: {similarity} scale_value: {scale_value} == NO ==")
        return {}

def compare_images_pil(image1_path, image2_path, ImageName, processed_files, scale_value, lock, update_listbox_queue):
    """ PIL과 scikit-image를 사용하여 두 이미지의 유사도를 비교하는 함수 """
    image1_origin, image1_gray = check_and_load_image_pil(image1_path)
    ...
    if similarity >= float(scale_value.value):    
        log_message(f"similarity: {similarity} scale_value: {scale_value} == OK ==")
        return update_dict(processed_files, scale_value, lock, update_listbox_queue, image2_path, ImageName, image2_origin, similarity);
    else:
        log_message(f"similarity: {similarity} scale_value: {scale_value} == NO ==")
        return {}
    
def process_file(png_file, file_path, log_queue, processed_files, scale_value, lock, update_listbox_queue):
    args = (log_queue, processed_files, scale_value, lock, update_listbox_queue)
    if file_path.endswith(".psd"):
        log_message(file_path, log_queue=log_queue)       
        return process_psd_file(file_path, png_file, args)
    elif file_path.endswith((".png", ".jpg", ".jpeg")):
        log_message(file_path, log_queue=log_queue)       
        return compare_images_pil(png_file, file_path, os.path.basename(file_path), processed_files, scale_value, lock, update_listbox_queue)
    #log_message("process_file End", log_queue=log_queue)       

def process_file_wrapper(png_file, file_path, log_queue, processed_files, scale_value, lock, update_listbox_queue, files_processed):
    files_processed.value += 1
    result = process_file(png_file, file_path, log_queue, processed_files, scale_value, lock, update_listbox_queue)
    # update?
    update_listbox_queue.put(result)
    return  result
...
def process_folder_parallel(png_file, folder_path, on_all_threads_completed, args):
    ...
    file_paths = [(png_file, os.path.join(root, filename), log_queue, processed_files, scale_value, lock, update_listbox_queue, files_processed) 
                  for root, _, files in os.walk(folder_path) 
                  for filename in files if filename.endswith((".psd", ".png", ".jpg", ".jpeg"))]

    pool = multiprocessing.Pool() #Pool(processes=availableProcess)
...
def fast_process_file(png_file, file_path, log_queue, processed_files, scale_value, lock, update_listbox_queue):
    args = (log_queue, processed_files, scale_value, lock, update_listbox_queue)
    if file_path.endswith(".psd"):
        log_message(file_path, log_queue=log_queue)       
...
def fast_process_file_wrapper(png_file, file_path, log_queue, processed_files, scale_value, lock, update_listbox_queue, files_processed):
    files_processed.value += 1
    result = fast_process_file(png_file, file_path, log_queue, processed_files, scale_value, lock, update_listbox_queue)
    update_listbox_queue.put(result)
    return result
...
def on_all_threads_completed(args, merged_dict):
    ...
    while not update_listbox_queue.empty():
        update_listbox_queue.get()
    label_status.config(text=f"completed")
    files_processed.value = total_files.value
    tkRoot.update_idletasks()
...
if __name__ == "__main__":
    ...
    update_listbox_queue = manager.Queue()
    #active_threads = 0
